...
export async function voiceAsk(promptText, listenSec = DEFAULT_LISTEN_SEC) {
  ...
  const customWords = vp?.custom_words || [
    ...
    "recruiter", "cold email", "compose", "kairo", "kairos"
  ];
...
`.trim();
    ...
    console.log(`\n  ${V.cyan}🎤 KAIRO Listening...${V.r} ${V.d}(Speak clearly into your mic or type text & press Enter)${V.r}`);
    ...
    sttProcess.on("exit", () => {
      ...
      if (text) {
        ...
        process.stdout.write(`\r  ${V.green}🎤 KAIRO heard:${V.r} "${V.b}${cleaned}${V.r}"\n`);
        finish(cleaned);
      } else {
        finish("");
      }
...
// ─── Wake Word Matching (KAIRO / Kairos) ─────────────────────────────
export function matchesWakeWord(text) {
  const lower = (text || "").toLowerCase().trim();
  if (/\bhello\s*(kairo|kairos|cairo|kyro|chiro)\b/i.test(lower)) return true;
  if (/\bhey\s*(kairo|kairos|cairo|kyro|chiro)\b/i.test(lower)) return true;
  if (/\bhi\s*(kairo|kairos|cairo)\b/i.test(lower)) return true;
  if (/\bok\s*(kairo|kairos|cairo)\b/i.test(lower)) return true;
  if (/^(kairo|kairos|cairo)\b/i.test(lower)) return true;
  return false;
}
  ...
  try {
    ...
    const resp = await axios.post(endpoint, {
      ...
      messages: [
        {
          role: "system",
          content: "You are an AI wake-word detector for assistant 'KAIRO' (also pronounced Kairos or Cairo). Given spoken text from audio speech-to-text, answer strictly 'YES' if the user intended to greet or wake up KAIRO (e.g., 'hello kairo', 'hey kairos', 'wake up kairo', 'hi assistant'), or 'NO' otherwise."
        },
  ...
  try {
    ...
    const resp = await axios.post(endpoint, {
      ...
      messages: [
        {
          role: "system",
          content: "You are a speech intent normalizer for JARVIS-style assistant KAIRO. Correct any misheard spoken words into a clean user action command for browser automation. Output ONLY the clean action command in 1 line."
        },
...
export async function voiceMenu(askFn) {
  console.log(`\n  ${V.bgMag}${V.b} 🤖 KAIRO — JARVIS Voice Mode ${V.r}\n`);
  ...
  const response = await askFn(
    `  ${V.cyan}🎤 KAIRO listening: What is your command?${V.r} `, 8
  );

  if (!response) {
    speak("Switching to manual input mode.");
    setVoiceMode(false);
  ...
  if (["job", "jobs", "apply", "hire", "recruiter", "career", "linkedin", "indeed", "naukri"]
    ...
    const role = response;
    const location = await askFn(`  ${V.cyan}Preferred location? Remote or city?${V.r} `, 5) || "Remote";
    speak(`Understood. Searching for ${role} positions in ${location} and preparing applications.`);
  ...
  if (["mail", "email", "cold mail", "outreach", "gmail", "message recruiter", "write email"]
    ...
    speak(`Opening Gmail to draft your outreach to ${recipient}.`);
  ...
  if (["buy", "shop", "order", "purchase", "laptop", "phone", "product", "amazon", "flipkart", "instacart", "blinkit", "zepto", "football"]
    ...
    if (!targetSite) {
      const siteAnswer = await askFn(`  ${V.cyan}Which platform? Amazon, Flipkart, or Google:${V.r} `, 5);
      if (siteAnswer) {
    ...
    speak(`Initiating search for ${cleanProduct} ${siteStr}.`);
  ...
  if (["search", "research", "find", "look up", "learn", "what is", "who is", "how to"]
    .some(kw => lower.includes(kw))) {
    const where = await askFn(`  ${V.cyan}Search platform? Google or Wikipedia:${V.r} `, 5) || "Google";
    speak(`Researching on ${where}. Stand by.`);
  ...
  if (["book", "ticket", "flight", "hotel", "train", "bus", "travel"]
    .some(kw => lower.includes(kw))) {
    const from = await askFn(`  ${V.cyan}Origin location?${V.r} `, 5) || "";
    const to = await askFn(`  ${V.cyan}Destination?${V.r} `, 5) || "";
    const date = await askFn(`  ${V.cyan}Travel date?${V.r} `, 5) || "";
    const dateStr = (date && !date.toLowerCase().includes("skip")) ? ` on ${date}` : "";
    speak(`Configuring travel plan from ${from} to ${to}${dateStr}.`);
  ...
  speak("Command acknowledged. Processing your request immediately.");
  return response;
}
...
export async function calibrateVoiceProfile() {
  console.log(`\n  ${V.bgMag}${V.b} 🎙️  KAIRO User Voice Calibration & Acoustic Training ${V.r}\n`);
  console.log(`  ${V.d}KAIRO will listen to sample phrases from your voice to calibrate its${V.r}`);
  console.log(`  ${V.d}JARVIS speech recognition engine, gain sensitivity, and custom vocabulary.${V.r}\n`);

  speak("Welcome to Voice Calibration. Let's tune KAIRO to recognize your voice with optimal precision.");

  const phrases = [
    "Hello KAIRO, order a football on Amazon under 400rs",
  ...
  for (let i = 0; i < phrases.length; i++) {
    ...
    speak(`Step ${i + 1}. Please read: ${target}`);
  ...
  console.log(`  ${V.d}KAIRO is now trained on your acoustic voice characteristics.${V.r}\n`);
  speak("Acoustic calibration complete. KAIRO systems are fully calibrated to your voice.");
